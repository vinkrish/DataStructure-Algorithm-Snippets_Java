...
public class GenomicRangeQuery {
	...
	static public int[] solution(String S, int[] P, int[] Q) {
		...
		ArrayList<Integer> gPositions = new ArrayList<>();
		ArrayList<Integer> tPositions = new ArrayList<>();
		
		for (int i=0; i<S.length(); i++) {
			if (S.charAt(i) == 'A') aPositions.add(i);
			else if (S.charAt(i) == 'C') cPositions.add(i);
			else if (S.charAt(i) == 'G') gPositions.add(i);
			else if (S.charAt(i) == 'T') tPositions.add(i);
		}
		...
		for (int i=0; i<P.length; i++) {
			...
			if (!Collections.disjoint(aPositions, temp)) {
				minimalImpactFactor[i] = 1;
			} else if (!Collections.disjoint(cPositions, temp)) {
				minimalImpactFactor[i] = 2;
			} else if (!Collections.disjoint(gPositions, temp)) {
				minimalImpactFactor[i] = 3;
			} else if (!Collections.disjoint(tPositions, temp)) {
